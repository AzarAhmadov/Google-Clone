...
const HomeSearch: FC = () => {
    ...
    const randomSearch = async () => {
        ...
        if (randomWords) {
            const searchTerm = randomWords[0];
    ...
    return (
        <section className='ps-3 pe-3 w-full flex flex-col items-center'>
            <form
                className='flex items-center w-full mt-5 mx-auto max-w-[90%] border border-gray-200 px-5 sm:px-4 sm:py-2 py-3 rounded-full hover:shadow-md focus-within:shadow-md transition-shadow sm:max-w-xl lg:max-w-2xl'
            ...
            <div className='flex flex-col space-y-2 md:space-y-0 justify-center md:flex-row mt-8 sm:mt-5 md:space-x-4'>
                ...
                <button disabled={randomLoading} onClick={randomSearch} className='bg-[#f8f9fa] flex items-center justify-center rounded-md text-sm text-gray-800 hover:ring-gray-200 focus:outline-none active:ring-gray-300 hover:shadow-md w-36 h-10 transition:shadow disabled:bg-[#00000007] disabled:shadow-sm'>
                    {
                        randomLoading ?
                            <div role="status">
